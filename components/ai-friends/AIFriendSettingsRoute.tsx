...
import { AIFriendSettingsPage } from "@/components/ai-friends/AIFriendSettingsPage";
...
export function AIFriendSettingsRoute({ groupId, initialGroup }: Props) {
  ...
  if (group) return <AIFriendSettingsPage group={group} />;
  ...
  return (
    <main className="app-backdrop grid min-h-dvh place-items-center px-4">
      <div className="w-full max-w-[340px] rounded-[20px] bg-cream px-5 py-6 text-center shadow-manor-lg border border-gold-200/20">
        <p className="text-sm font-semibold text-ink-deep">{loaded ? "找不到这个群聊" : "正在打开..."}</p>
